...
with col1:
    ...
    # Selezione del modo fuori dal form o gestita con callback per rendere reattivo l'RST
    banda = st.selectbox(
        "Banda",
        [
            "160m",
    ...
    # Determiniamo l'RST automatico in base al modo
    if modo == "CW":
        default_rst = "599"
    ...
    # Form per l'inserimento dei dati del QSO
    with st.form("qso_form", clear_on_submit=True):
        ...
        with col_l1:
            wwloc_miei = st.text_input(
                "Mio WWLOC (Locator)", placeholder="es. JN65rt"
            ).strip()
        with col_l2:
        ...
        if submitted:
            if station and nominativo:
                ...
                if wwloc_miei and wwloc_dx:
                    dist, az = calculate_distance_and_azimuth(
                        wwloc_miei, wwloc_dx
                    )
