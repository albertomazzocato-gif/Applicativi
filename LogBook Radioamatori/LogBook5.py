...
with st.form("qso_form", clear_on_submit=False):
    st.subheader("Inserisci Nuovo QSO")

    col1, col2 = st.columns(2)

    with col1:
        # Nessun blocco dell'operatore fisso per la versione web condivisa
        my_callsign = st.text_input(
            "Tuo Nominativo (Operatore)", value="", placeholder="es. IZ3XXX"
        )
    ...
    # Gestione automatica dell'RST in base al modo scelto (richiesta presidente)
    default_rst = "59"
    if mode == "CW":
        default_rst = "599"
    elif mode in ["FT8", "FT4"]:
        default_rst = "00"  # o -10, impostato su standard rapido richiesto
    ...
    dx_locator = st.text_input(
        "Locatore Corrispondente (DX)", value="", placeholder="es. JN55"
    ).upper()
    ...
    if submitted:
        if not callsign:
            ...
            # Calcolo distanza e azimut se entrambi i locatori sono presenti
            dist, az = None, None
...
# Visualizzazione della tabella dei QSO
if st.session_state.log_entries:
    st.divider()
    st.subheader(f"📋 Lista QSO ({len(st.session_state.log_entries)})")
