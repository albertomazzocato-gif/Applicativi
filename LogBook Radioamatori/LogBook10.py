...
def salva_qso(
    ...
    data = {
        "Data/Ora": datetime.now(ZoneInfo("Europe/Rome")).strftime("%d-%m-%Y %H:%M"),
    ...
    if os.path.exists(csv_temp):
        ...
        # Se nel CSV vecchio ci sono colonne spurie o doppie, le ripuliamo tenendo solo quelle corrette
        colonne_attese = list(data.keys())
        for col in colonne_attese:
            if col not in log.columns:
                log[col] = "-"
        log = log[colonne_attese]
...
with col2:
    ...
    if os.path.exists("temp_log.csv"):
        ...
        # Filtra eventuali colonne doppie o vecchie rimaste nel file csv
        colonne_attese = ["Data/Ora", "Stazione", "DX", "Banda", "Modo", "RST In", "RST Out", "Mio Loc", "DX Loc", "Distanza", "Azimuth", "Note"]
        for col in colonne_attese:
